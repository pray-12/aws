# Highly Available and Scalable Multi-AZ VPC Architecture on AWS

## Project Overview

This project demonstrates how to create a highly available and scalable multi-AZ VPC architecture on AWS.

To improve resiliency, we deploy the servers in two Availability Zones, by using an Auto Scaling group and an Application Load Balancer. For additional security, we deploy the servers in private subnets. The servers receive requests through the load balancer. The servers can connect to the internet by using a NAT gateway.

- The VPC has public subnets and private subnets in two Availability Zones.

- Each public subnet contains a NAT gateway and a load balancer node.

- The servers run in the private subnets, are launched and terminated by using an Auto Scaling group, and receive traffic from the load balancer.

- The servers can connect to the internet by using the NAT gateway.

## Project Architecture

![Project Architecture](images/architecture-diagram.png)

## Project Implementation

### Create a VPC

1. Go to the AWS Management Console.
2. Navigate to the VPC service.
3. Click on "Create VPC".
4. Enter the following details:
    - select vpc and more options.
    - Name: "project"
    - CIDR block: "10.0.0.0/16"
    - Availability Zones: "ap-south-1a, ap-south-1b"
    - Nat Gateway: " 1 per AZ"
    - VPC Endpoint: "None"
    - Click on "Create VPC".

![Create VPC](/images/vpc-creation.png)
<br/>
![Create VPC](/images/create-vpc-workflow.png)

### Create a auto scaling group

1. Go to the AWS Management Console.
2. Navigate to the EC2 service.
3. Click on "Auto Scaling Groups".
![Auto Scaling Group](/images/asg-1.png)
4. Create a Launch Template.
  - Give Launch template name and description
    ![](/images/asg-2.png)
  - give Application and OS Images (Amazon Machine Image) - required and Instance Type as "t3.micro"
   ![Launch Template](/images/asg-3.png)
  -  Provide key pair and network settings
    ![Launch Template](/images/asg-4.png)
  - Create Launch Template.
5. Once the launch template is created, select it in Launch template of Auto Scaling Group.
  ![Auto Scaling Group](/images/asg-5.png)
6. Choose instance launch option here select the 'vpc' and availability zones as "ap-south-1a, ap-south-1b" which is private subnets.
  ![](/images/asg-6.png)
7. Configure group size and scaling
    - Desired capacity: 2
    - Minimum capacity: 1
    - Maximum capacity: 4
    ![](/images/asg-7.png)
8. Launch configuration created successfully.
9. When we click on EC2 we will see two instances in the private subnets.
![](images/asg-8.png)
10. These instance will not have a public ip address because they are in the private subnets.
11. We will create a bastion-host[A bastion host (also called a jump server) is a secure entry point used to access instances that are inside a private subnet.] in the public subnet to connect to the instances.
    - Go to the AWS Management Console.
    - Navigate to the EC2 service.
    - Click on "Instances".
    - Click on "Launch Instance".
    - DO the configuration and make sure in network settings vpc is selected as "project" and subnet is selected as "public-subnet".
    - Auto-assign public ip: "enable"
    - Create instance.
    - Once the instance is created, we will see the instance in the public subnet.

12. Connect the bastion host to the instances in private subnets.
```bash
/mnt/c/Windows/explorer.exe . # to open the file explorer.
udo scp -i /home/preethy/aws-instance/key-pair.pem /home/preethy/aws-instance/key-pair.pem ubuntu@18.60.157.139
:/home/ubuntu
ssh -i "my-key-pair.pem" ubuntu@bastion-host-public-ip
```
13. Create a simple index.html file in the home directory.
```bash
echo "<h1>Hello World</h1>" > index.html
```
14. Start a simple http server.
```bash
python3 -m http.server 8000
```
15. Now connect to one of the private instances and check the index.html file.
```bash
ssh -i "my-key-pair.pem" ubuntu@private-instance-public-ip
cat index.html
```
15. Now create a Load Balancer.

16. Before that need to create `target group` for the load balancer.
    ![](images/image-9.png)
17. Register targets 
    ![](images/image-10.png)
18. Create a Load Balancer.
    ![](images/image-11.png)
19. Select the target group and create a Load Balancer.
    ![](images/image-12.png)

