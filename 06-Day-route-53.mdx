...
👉 It translates human-readable domain names like `example.com` into IP addresses like `192.0.2.1`.


### 🔹 Why is it called “Route 53”?

- “Route” → routes user traffic  
- “53” → DNS uses **port 53**

## 🧠 Core Functions of Route 53

### 1. Domain Registration
- You can **register domain names** directly in Route 53
- Example: `myapp.com`

---

### 2. DNS Routing
- Maps domain names → IP addresses or AWS resources
- Example:
  - `www.example.com` → EC2 / Load Balancer

---

### 3. Health Checking
- Monitors endpoint health
- Automatically reroutes traffic if a resource becomes unhealthy

---

### 4. Traffic Management
- Routes traffic based on rules like:
  - Latency
  - Geography
  - Weight
  - Failover
...
- **Private Hosted Zone**
  - Used inside VPC (internal apps)

---

### 🔹 DNS Records

Records define how traffic is routed.
...
- Maps domain to AWS resources:
  - Load Balancer
  - CloudFront
  - S3
- No extra cost
- Better than CNAME for AWS services

## 🚀 Routing Policies (INTERVIEW FAVORITE ⭐)
...
👉 Used for testing / canary deployments
...
- Routes based on user location (country)
...
### Behavior:
- If endpoint fails → removed from DNS response
...
## 🎯 Real-Time Scenario (Interview Answer ⭐)

“Suppose I have an application deployed in two regions:

- I configure **Latency-Based Routing** in Route 53  
- Add health checks to both endpoints  

👉 If one region goes down, Route 53 automatically routes traffic to the healthy region with lowest latency."

Route 53 is a scalable DNS and traffic routing service that supports multiple routing policies, health checks, and domain management to ensure high availability and low latency.

### Pro Tips for Interview

- Alias record is AWS-specific (very important)
- Route 53 is **global service**
- Health checks + routing policies = high availability
- Used for **disaster recovery strategies**

## 🔥 Advanced Concepts

### 🔹 TTL (Time To Live)
- Defines how long DNS response is cached
...
### 🔹 Split-Horizon DNS
- Same domain:
  - Different response inside VPC vs outside
...
- Supports **DNSSEC** (Domain Name System Security Extensions)
