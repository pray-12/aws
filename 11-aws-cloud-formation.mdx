...
3. Create a stack
4. CloudFormation calls AWS APIs
5. Resources get created (EC2, S3, VPC, etc.)

```yaml
AWSTemplateFormatVersion: "2010-09-09"   # Template version

Description: Simple EC2 instance         # What this template does

Resources:                               # Main section (must have)
  MyEC2Instance:                         # Logical name
    Type: AWS::EC2::Instance             # Resource type
    Properties:
      InstanceType: t2.micro             # Instance size
      ImageId: ami-12345678              # AMI ID (example)
```

“This is a basic CloudFormation template where the Resources section defines an EC2 instance with a specific instance type and AMI. When we create a stack, AWS automatically provisions this instance.”

```yaml
AWSTemplateFormatVersion: "2010-09-09"   # Template version

Description: Simple S3 Bucket            # Purpose of template

Resources:                               # Mandatory section
  MyS3Bucket:                            # Logical name
    Type: AWS::S3::Bucket                # Resource type (S3 bucket)
    Properties:
      BucketName: my-unique-bucket-12345 # Bucket name (must be globally unique)
      VersioningConfiguration:
        Status: Enabled
```

This CloudFormation template creates an S3 bucket by defining it in the Resources section. When the stack is created, AWS provisions the bucket automatically.


- **Parameters** → reusability
- **Mappings** → region handling
- **Conditions** → environment control
- **Resources** → actual infra
...
##  Stack Concept (Very Important)

👉 A stack = running instance of template

- You don’t create resources directly
- You create a stack, which manages everything
- **Stack operations**:
  - Create
  - Update
  - Delete

## Change Sets (Interview Gold 🔥)

👉 Before applying changes:

**CloudFormation shows**:
  - What will be created
  - Modified
  - Deleted

“It’s like a preview before execution[under Events]”

## Drift Detection [Very Important]

👉 Detects if someone manually changed resources outside CloudFormation

Example:

- You created EC2 via template
- Someone changed instance type manually
...
CloudFormation automatically deletes created resources
Restores previous stable state

👉 This is a big advantage over manual setup
...
AWS CloudFormation is an Infrastructure as Code service used to provision and manage AWS resources using templates. It provides full control over infrastructure such as EC2, VPC, IAM, Load Balancers, and databases.

AWS Elastic Beanstalk is a Platform as a Service that simplifies application deployment. Developers upload application code, and AWS automatically manages infrastructure, scaling, load balancing, and monitoring.

CloudFormation is preferred when full infrastructure customization and automation are required, while Elastic Beanstalk is ideal for quickly deploying applications without managing infrastructure manually.
