AWS Secrets Manager is a **managed service for securely storing, retrieving, and managing sensitive information** like:
...
Instead of hardcoding secrets in your code or config files, you store them safely in AWS and access them when needed.

We have 3 ways to store the secrets:
1. **Secrets Manager**: This is used for storing sensitive data like passwords and API keys with built-in rotation.
2. **Systems Manager or Parameter Store**: This is a simpler and cheaper option mainly for configuration values and basic secrets without rotation.
3. **HashiCorp Vault**: Not a AWS service, but it is a popular tool for storing and managing secrets.

“AWS Secrets Manager is used for storing sensitive data like passwords and API keys with built-in rotation. Parameter Store is a simpler and cheaper option mainly for configuration values and basic secrets without rotation. HashiCorp Vault is an external tool used in multi-cloud or enterprise environments with advanced secret management and dynamic credentials.

- Secrets Manager → Secure + Rotation[example: database password , rotating meaning changing the password after a certain period of time]
- Parameter Store → Simple + Free[example: API key, If we don'nt want to store much sensitive data, we can use parameter store]
- Vault → Powerful + Multi-cloud[example: multi-cloud secret management, if we want to store more sensitive data, we can use vault]
